...
# [Poll](https://forms.gle/8RzHoTMpUkpYeSUT9)
...
```{r}
lables <- c(emo::ji("no"),
            emo::ji("person shrugging"), 
            emo::ji("jack-o-lantern"))

labels <- c("Definitely", "IDK", "Nope")

subdat <- select(data, x = 5) %>%
  mutate(x = ifelse(substr(x, 1, 1) == "D", "Definitely")) %>%
  mutate(x = factor(x, 
...
ggplot(subdat, aes(x = x, fill = x)) +
  ...
  scale_x_discrete(name = "",  drop=FALSE) +
  scale_y_continuous(name = "", breaks = 0:11*5) +
  scale_fill_manual(values = "darkorange", "white", "black") +
  coord_flip(ylim = c(0, max(n, 5))) +
