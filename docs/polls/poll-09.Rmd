...
data <- read_sheet(url, sheet = "Week9") %>%
  filter(row_number() != 0)
...
### How did you engage with Chapter 9?
...
subdat <- select(data, x = 5) %>%
  filter(!is.na(x)) %>%
  separate_rows(x, sep = ", ") %>%
  mutate(x = factor(x, resp))

ggplot(subdat, aes(x = x, fill = x)) +
  geom_bar(show.legend = FALSE, colour = "black") +
  scale_x_discrete(name = "",  drop=FALSE, limits = rev) +
  scale_y_continuous(name = "", breaks = 0:11*5) +
...
```{r}
subdat <- select(data, x = 7) %>%
  filter(!is.na(x)) %>%
  separate_rows(x, sep = ", ") %>%
  separate(x, into = c("date", "chapter", "topic"), sep = " - ")

ggplot(subdat, aes(x = chapter, fill = chapter)) +
  geom_bar(show.legend = FALSE, colour = "black") +
  scale_x_discrete(name = "",  drop=FALSE, limits = rev) +
  scale_y_continuous(name = "", breaks = 0:11*5) +
  scale_fill_brewer(palette = "Spectral") +
...
### List concepts you learned in Chapter 9 that you feel confident about.
...
### List concepts you learned in Chapter 9 that you feel less confident about and would like reviewed in class today.
